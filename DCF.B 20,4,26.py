...
def out_graph_surf(ticker_str,w,h):
    try:
        ...
        ax.set_title(f"{ticker_str} : 5 Year Stock Performance",color='white')
...
def ui (screen,mainClock):
    ...
    def inp_box(screen, x, y, w, h, text, font_size=15):  # Builds a UI input box
        ...
        inp_rect = pygame.Rect(x, y, w,
                               h)  # Takes an input to draw a rectangle of the given position, x & y as well as, width and height
        pygame.draw.rect(screen, (128, 128, 128), inp_rect)  # Draws the rectangle with the colour gray

        out_colour = (255, 255, 255) if inp_rect.collidepoint(pygame.mouse.get_pos()) else (0, 0,
                                                                                            0)  # If the mouse is touching the button, its outline will be white
